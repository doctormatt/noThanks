...
for game in range(numberOfGames): ## main game loop
	...
	for i in range (numberOfPlayers):
		players[i] = Player(11,[]) # initialize each player with 11 tokens and an empty list of cards
	...
	while(cardIndex<24):
		...
		#print currentCard," ",currentTokens," ",currentPlayer," ",players[currentPlayer].tokens," ",players[currentPlayer].cards
		...
		if (currentPlayer in [0]):	
			...
			# (2) currentCard-currentTokens<15, so the number of tokens offsets the points of the cards sufficiently, or
			# (3) the player has no tokens
			if (  (currentCard+1 in players[currentPlayer].cards) or (currentCard-1 in players[currentPlayer].cards) or (currentCard-currentTokens<15) or (players[currentPlayer].tokens==0)):
		...
		if (currentPlayer in [1,2,3]):
			# this next conditional statement is the strategy for this/these players:
			# pick up the card if (1) is a adjacent to a card the player already has, or
			# (2) currentCard-currentTokens<20, so the number of tokens offsets the points of the cards sufficiently, or
			# (3) the player has no tokens
			if ( (currentCard+1 in players[currentPlayer].cards) or (currentCard-1 in players[currentPlayer].cards) or (currentCard-currentTokens<20) or (players[currentPlayer].tokens==0)):
			 ## currentPlayer takes the card and the tokens, if any
				players[currentPlayer].tokens = players[currentPlayer].tokens+currentTokens
				players[currentPlayer].cards.append(currentCard)
				cardIndex=cardIndex+1 ## get ready to turn over the next card
	...
	for i in range(numberOfPlayers):
		...
		for j in range(len(players[i].cards)):	
			# the lowest cards counts, and all other cards count only if they are not one more
	...
	for i in range(numberOfPlayers):
		...
		#print "bleh ",i," ",players[i].tokens," ",players[i].cards," ",scores[i]

	# print out current wins every so often
